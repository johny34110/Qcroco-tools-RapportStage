...
\begin{figure}[H]
	...
	\label{fig:QGIS}
\end{figure}
QGIS prend en charge une variété de formats de données, y compris les formats vectoriels et raster, et offre une gamme d'outils pour la visualisation de données, l'analyse spatiale, la gestion de données et plus encore (\cite{QGIS}). Il est également extensible grâce à une large gamme de plugins, ce qui permet aux utilisateurs d'ajouter des fonctionnalités supplémentaires en fonction de leurs besoins spécifiques. Dans le cadre de ce travail, un plugin spécifique, nommé QCrocoFlow, a été développé par mon maître de stage pour faciliter la manipulation des données nécessaires pour le modèle CROCO.
